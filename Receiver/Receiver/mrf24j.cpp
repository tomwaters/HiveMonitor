 ...
 * from https://github.com/karlp/Mrf24j40-arduino-library
 * modified for Raspberry Pi using WiringPi
 */
 ...
 * @param pin_reset, @param pin_chip_select, @param spi_channel
 */
Mrf24j::Mrf24j(int pin_reset, int pin_chip_select, int spi_channel) {
    ...
    wiringPiSPISetup(_spi_channel, 10000000);
}
...
byte Mrf24j::read_short(byte address) {
    ...
    byte b1 = address<<1 & 0b01111110;
    wiringPiSPIDataRW(_spi_channel, &b1, 1);

    byte ret = 0x00;
    wiringPiSPIDataRW(_spi_channel, &ret, 1);
...
byte Mrf24j::read_long(word address) {
    ...
    byte b1 = 0x80 | ahigh;
    wiringPiSPIDataRW(_spi_channel, &b1, 1);
    wiringPiSPIDataRW(_spi_channel, &alow, 1);
...
void Mrf24j::write_short(byte address, byte data) {
    ...
    byte b1 = (address<<1 & 0b01111110) | 0x01;
    wiringPiSPIDataRW(_spi_channel, &b1, 1);
...
void Mrf24j::write_long(word address, byte data) {
    ...
    byte b2 = alow | 0x10;
    wiringPiSPIDataRW(_spi_channel, &b1, 1);
    wiringPiSPIDataRW(_spi_channel, &b2, 1);
    wiringPiSPIDataRW(_spi_channel, &data, 1);
...
void Mrf24j::set_palna(boolean enabled) {
    if (enabled) {
        ...
        write_short(MRF_TRISGPIO,0b00111111);
        write_short(MRF_GPIO,0b00001111);
    }else{
